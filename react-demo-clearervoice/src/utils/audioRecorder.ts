 ...
 * Records both unprocessed (before Speex) and processed (after Speex) audio
 */
...
export class AudioRecorder {
  ...
  async startRecording(
    ...
    try {
      ...
      console.log('[RECORDER] ✅ Started recording both raw and processed audio', {
        speexEnabled,
        browserNSEnabled,
        sampleRate
      });
...
export function downloadRecording(blob: Blob, filename: string): void {
  ...
  console.log(`[RECORDER] Downloaded recording: ${filename}`);
}
...
export function downloadRecordingWithMetadata(
  ...
  console.log('[RECORDER] ✅ Downloaded all recordings and metadata');
}
